...
*/
// custom definitions
if (!\defined('OCA\JournalNotes\Vendor\DATAMATRIXDEFS')) {
    /**
     * Indicate that definitions for this class are set
     */
    \define('OCA\JournalNotes\Vendor\DATAMATRIXDEFS', \true);
    // -----------------------------------------------------
}
...
class Datamatrix
{
     ...
     * @protected
     */
    protected $last_enc = \OCA\JournalNotes\Vendor\ENC_ASCII;
    /**
     ...
     * @protected
     */
    protected $chset_id = array(\OCA\JournalNotes\Vendor\ENC_C40 => 'C40', \OCA\JournalNotes\Vendor\ENC_TXT => 'TXT', \OCA\JournalNotes\Vendor\ENC_X12 => 'X12');
    /**
    ...
    public function __construct($code)
    {
        ...
        if ($params[11] < $nd) {
            ...
            if ($params[11] - $nd > 1 and $cw[$nd - 1] != 254) {
                if ($this->last_enc == \OCA\JournalNotes\Vendor\ENC_EDF) {
                    // switch to ASCII encoding
                    $cw[] = 124;
                    ++$nd;
                } elseif ($this->last_enc != \OCA\JournalNotes\Vendor\ENC_ASCII and $this->last_enc != \OCA\JournalNotes\Vendor\ENC_BASE256) {
                    // switch to ASCII encoding
                    $cw[] = 254;
                    ++$nd;
                }
    ...
    protected function lookAheadTest($data, $pos, $mode)
    {
        ...
        // STEP J
        if ($mode == \OCA\JournalNotes\Vendor\ENC_ASCII) {
        ...
        while (\true) {
            ...
            if ($pos + $charscount == $data_length) {
                if ($numch[\OCA\JournalNotes\Vendor\ENC_ASCII] <= \ceil(\min($numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256]))) {
                    return \OCA\JournalNotes\Vendor\ENC_ASCII;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_BASE256] < \ceil(\min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_EDF]))) {
                    return \OCA\JournalNotes\Vendor\ENC_BASE256;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_EDF] < \ceil(\min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256]))) {
                    return \OCA\JournalNotes\Vendor\ENC_EDF;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_TXT] < \ceil(\min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256]))) {
                    return \OCA\JournalNotes\Vendor\ENC_TXT;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_X12] < \ceil(\min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256]))) {
                    return \OCA\JournalNotes\Vendor\ENC_X12;
                }
                return \OCA\JournalNotes\Vendor\ENC_C40;
            }
            // get char
            $chr = \ord($data[$pos + $charscount]);
            $charscount++;
            // STEP L
            if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_NUM)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_ASCII] += 1 / 2;
            } elseif ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_EXT)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_ASCII] = \ceil($numch[\OCA\JournalNotes\Vendor\ENC_ASCII]);
                $numch[\OCA\JournalNotes\Vendor\ENC_ASCII] += 2;
            } else {
                $numch[\OCA\JournalNotes\Vendor\ENC_ASCII] = \ceil($numch[\OCA\JournalNotes\Vendor\ENC_ASCII]);
                $numch[\OCA\JournalNotes\Vendor\ENC_ASCII] += 1;
            }
            // STEP M
            if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_C40)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_C40] += 2 / 3;
            } elseif ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_EXT)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_C40] += 8 / 3;
            } else {
                $numch[\OCA\JournalNotes\Vendor\ENC_C40] += 4 / 3;
            }
            // STEP N
            if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_TXT)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_TXT] += 2 / 3;
            } elseif ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_EXT)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_TXT] += 8 / 3;
            } else {
                $numch[\OCA\JournalNotes\Vendor\ENC_TXT] += 4 / 3;
            }
            // STEP O
            if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_X12) or $this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_C40)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_X12] += 2 / 3;
            } elseif ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_EXT)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_X12] += 13 / 3;
            } else {
                $numch[\OCA\JournalNotes\Vendor\ENC_X12] += 10 / 3;
            }
            // STEP P
            if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_EDF)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_EDF] += 3 / 4;
            } elseif ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_EXT)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_EDF] += 17 / 4;
            } else {
                $numch[\OCA\JournalNotes\Vendor\ENC_EDF] += 13 / 4;
            }
            // STEP Q
            if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_BASE256)) {
                $numch[\OCA\JournalNotes\Vendor\ENC_BASE256] += 4;
            } else {
                $numch[\OCA\JournalNotes\Vendor\ENC_BASE256] += 1;
            }
            // STEP R
            if ($charscount >= 4) {
                if ($numch[\OCA\JournalNotes\Vendor\ENC_ASCII] + 1 <= \min($numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256])) {
                    return \OCA\JournalNotes\Vendor\ENC_ASCII;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_BASE256] + 1 <= $numch[\OCA\JournalNotes\Vendor\ENC_ASCII] or $numch[\OCA\JournalNotes\Vendor\ENC_BASE256] + 1 < \min($numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_EDF])) {
                    return \OCA\JournalNotes\Vendor\ENC_BASE256;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_EDF] + 1 < \min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256])) {
                    return \OCA\JournalNotes\Vendor\ENC_EDF;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_TXT] + 1 < \min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_X12], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256])) {
                    return \OCA\JournalNotes\Vendor\ENC_TXT;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_X12] + 1 < \min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_C40], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256])) {
                    return \OCA\JournalNotes\Vendor\ENC_X12;
                }
                if ($numch[\OCA\JournalNotes\Vendor\ENC_C40] + 1 < \min($numch[\OCA\JournalNotes\Vendor\ENC_ASCII], $numch[\OCA\JournalNotes\Vendor\ENC_TXT], $numch[\OCA\JournalNotes\Vendor\ENC_EDF], $numch[\OCA\JournalNotes\Vendor\ENC_BASE256])) {
                    if ($numch[\OCA\JournalNotes\Vendor\ENC_C40] < $numch[\OCA\JournalNotes\Vendor\ENC_X12]) {
                        return \OCA\JournalNotes\Vendor\ENC_C40;
                    }
                    if ($numch[\OCA\JournalNotes\Vendor\ENC_C40] == $numch[\OCA\JournalNotes\Vendor\ENC_X12]) {
                        $k = $pos + $charscount + 1;
                        while ($k < $data_length) {
                            $tmpchr = \ord($data[$k]);
                            if ($this->isCharMode($tmpchr, \OCA\JournalNotes\Vendor\ENC_X12)) {
                                return \OCA\JournalNotes\Vendor\ENC_X12;
                            } elseif (!($this->isCharMode($tmpchr, \OCA\JournalNotes\Vendor\ENC_X12) or $this->isCharMode($tmpchr, \OCA\JournalNotes\Vendor\ENC_C40))) {
                                break;
                            }
                            ++$k;
                        }
    ...
    protected function getHighLevelEncoding($data)
    {
        ...
        $enc = \OCA\JournalNotes\Vendor\ENC_ASCII;
                ...
                case \OCA\JournalNotes\Vendor\ENC_ASCII:
                    {
                        ...
                        if ($data_length > 1 and $pos < $data_length - 1 and ($this->isCharMode(\ord($data[$pos]), \OCA\JournalNotes\Vendor\ENC_ASCII_NUM) and $this->isCharMode(\ord($data[$pos + 1]), \OCA\JournalNotes\Vendor\ENC_ASCII_NUM))) {
                            ...
                            if ($newenc != $enc) {
                                ...
                                $chr = \ord($data[$pos]);
                                ++$pos;
                                if ($this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_ASCII_EXT)) {
                ...
                case \OCA\JournalNotes\Vendor\ENC_X12:
                    {
                        ...
                        do {
                            ...
                            if ($chr & 0x80) {
                                if ($enc == \OCA\JournalNotes\Vendor\ENC_X12) {
                                    return \false;
                                }
                                $chr = $chr & 0x7f;
                                $temp_cw[] = 1;
                            ...
                            if (isset($charset[$chr])) {
                                ...
                                } elseif ($enc == \OCA\JournalNotes\Vendor\ENC_C40 and isset($this->chset['S3C'][$chr])) {
                                    $temp_cw[] = 2;
                                    // shift 3
                                    $shiftset = $this->chset['S3C'];
                                } elseif ($enc == \OCA\JournalNotes\Vendor\ENC_TXT and isset($this->chset['S3T'][$chr])) {
                                    $temp_cw[] = 2;
                                    // shift 3
                            ...
                            if ($p >= 3) {
                                ...
                                if ($newenc != $enc) {
                                    // switch to new encoding
                                    $enc = $newenc;
                                    if ($enc != \OCA\JournalNotes\Vendor\ENC_ASCII) {
                                        // set unlatch character
                                        $cw[] = $this->getSwitchEncodingCodeword(\OCA\JournalNotes\Vendor\ENC_ASCII);
                                        ++$cw_num;
                                    }
                                    $cw[] = $this->getSwitchEncodingCodeword($enc);
                                    ++$cw_num;
                        ...
                        if ($p > 0) {
                            ...
                            if ($cwr == 1 and $p == 1) {
                                ...
                                $enc = \OCA\JournalNotes\Vendor\ENC_ASCII;
                                $this->last_enc = $enc;
                            } else if ($enc != \OCA\JournalNotes\Vendor\ENC_ASCII) {
                                $enc = \OCA\JournalNotes\Vendor\ENC_ASCII;
                                $this->last_enc = $enc;
                                $cw[] = $this->getSwitchEncodingCodeword($enc);
                                ++$cw_num;
                ...
                case \OCA\JournalNotes\Vendor\ENC_EDF:
                    {
                        ...
                        do {
                            ...
                            if ($field_length == 4 or $epos == $data_length or !$this->isCharMode($chr, \OCA\JournalNotes\Vendor\ENC_EDF)) {
