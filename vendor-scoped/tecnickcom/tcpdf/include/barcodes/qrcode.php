 ...
 */
// definitions
if (!\defined('OCA\JournalNotes\Vendor\QRCODEDEFS')) {
    /**
     * Indicate that definitions for this class are set
     */
    \define('OCA\JournalNotes\Vendor\QRCODEDEFS', \true);
    // -----------------------------------------------------
    // Encoding modes (characters which can be encoded in QRcode)
    /**
     * Encoding mode
     */
    \define('OCA\JournalNotes\Vendor\QR_MODE_NL', -1);
    /**
     * Encoding mode numeric (0-9). 3 characters are encoded to 10bit length. In theory, 7089 characters or less can be stored in a QRcode.
     */
    \define('OCA\JournalNotes\Vendor\QR_MODE_NM', 0);
    /**
     * Encoding mode alphanumeric (0-9A-Z $%*+-./:) 45characters. 2 characters are encoded to 11bit length. In theory, 4296 characters or less can be stored in a QRcode.
     */
    \define('OCA\JournalNotes\Vendor\QR_MODE_AN', 1);
    /**
     * Encoding mode 8bit byte data. In theory, 2953 characters or less can be stored in a QRcode.
     */
    \define('OCA\JournalNotes\Vendor\QR_MODE_8B', 2);
    /**
     * Encoding mode KANJI. A KANJI character (multibyte character) is encoded to 13bit length. In theory, 1817 characters or less can be stored in a QRcode.
     */
    \define('OCA\JournalNotes\Vendor\QR_MODE_KJ', 3);
    /**
     * Encoding mode STRUCTURED (currently unsupported)
     */
    \define('OCA\JournalNotes\Vendor\QR_MODE_ST', 4);
    // -----------------------------------------------------
     ...
     */
    \define('OCA\JournalNotes\Vendor\QR_ECLEVEL_L', 0);
    /**
     * Error correction level M : About 15% or less errors can be corrected.
     */
    \define('OCA\JournalNotes\Vendor\QR_ECLEVEL_M', 1);
    /**
     * Error correction level Q : About 25% or less errors can be corrected.
     */
    \define('OCA\JournalNotes\Vendor\QR_ECLEVEL_Q', 2);
    /**
     * Error correction level H : About 30% or less errors can be corrected.
     */
    \define('OCA\JournalNotes\Vendor\QR_ECLEVEL_H', 3);
    // -----------------------------------------------------
     ...
     */
    \define('OCA\JournalNotes\Vendor\QRSPEC_VERSION_MAX', 40);
    /**
     * Maximum matrix size for maximum version (version 40 is 177*177 matrix).
     */
    \define('OCA\JournalNotes\Vendor\QRSPEC_WIDTH_MAX', 177);
    // -----------------------------------------------------
    /**
     * Matrix index to get width from $capacity array.
     */
    \define('OCA\JournalNotes\Vendor\QRCAP_WIDTH', 0);
    /**
     * Matrix index to get number of words from $capacity array.
     */
    \define('OCA\JournalNotes\Vendor\QRCAP_WORDS', 1);
    /**
     * Matrix index to get remainder from $capacity array.
     */
    \define('OCA\JournalNotes\Vendor\QRCAP_REMINDER', 2);
    /**
     * Matrix index to get error correction level from $capacity array.
     */
    \define('OCA\JournalNotes\Vendor\QRCAP_EC', 3);
    // -----------------------------------------------------
    // Structure (currently usupported)
    /**
     * Number of header bits for structured mode
     */
    \define('OCA\JournalNotes\Vendor\STRUCTURE_HEADER_BITS', 20);
    /**
     * Max number of symbols for structured mode
     */
    \define('OCA\JournalNotes\Vendor\MAX_STRUCTURED_SYMBOLS', 16);
    // -----------------------------------------------------
    // Masks
    /**
     * Down point base value for case 1 mask pattern (concatenation of same color in a line or a column)
     */
    \define('OCA\JournalNotes\Vendor\N1', 3);
    /**
     * Down point base value for case 2 mask pattern (module block of same color)
     */
    \define('OCA\JournalNotes\Vendor\N2', 3);
    /**
     * Down point base value for case 3 mask pattern (1:1:3:1:1(dark:bright:dark:bright:dark)pattern in a line or a column)
     */
    \define('OCA\JournalNotes\Vendor\N3', 40);
    /**
     * Down point base value for case 4 mask pattern (ration of dark modules in whole)
     */
    \define('OCA\JournalNotes\Vendor\N4', 10);
    // -----------------------------------------------------
    // Optimization settings
    /**
     * if true, estimates best mask (spec. default, but extremally slow; set to false to significant performance boost but (propably) worst quality code
     */
    \define('OCA\JournalNotes\Vendor\QR_FIND_BEST_MASK', \true);
    /**
     * if false, checks all masks available, otherwise value tells count of masks need to be checked, mask id are got randomly
     */
    \define('OCA\JournalNotes\Vendor\QR_FIND_FROM_RANDOM', 2);
    /**
     * when QR_FIND_BEST_MASK === false
     */
    \define('OCA\JournalNotes\Vendor\QR_DEFAULT_MASK', 2);
    // -----------------------------------------------------
}
// end of definitions
/**
...
class QRcode
{
     ...
     * @protected
     */
    protected $level = \OCA\JournalNotes\Vendor\QR_ECLEVEL_L;
    /**
     * Encoding mode.
     * @protected
     */
    protected $hint = \OCA\JournalNotes\Vendor\QR_MODE_8B;
    /**
    ...
    public function __construct($code, $eclevel = 'L')
    {
        ...
        if ($this->level === \false) {
            $this->level = \OCA\JournalNotes\Vendor\QR_ECLEVEL_L;
        }
        if ($this->hint != \OCA\JournalNotes\Vendor\QR_MODE_8B and $this->hint != \OCA\JournalNotes\Vendor\QR_MODE_KJ) {
            return \false;
        }
        if ($this->version < 0 or $this->version > \OCA\JournalNotes\Vendor\QRSPEC_VERSION_MAX) {
            return \false;
        }
        $this->items = array();
    ...
    protected function encodeMask($mask)
    {
        ...
        // masking
        $this->runLength = \array_fill(0, \OCA\JournalNotes\Vendor\QRSPEC_WIDTH_MAX + 1, 0);
        if ($mask < 0) {
            if (\OCA\JournalNotes\Vendor\QR_FIND_BEST_MASK) {
                $masked = $this->mask($this->width, $this->frame, $this->level);
            } else {
                $masked = $this->makeMask($this->width, $this->frame, \intval(\OCA\JournalNotes\Vendor\QR_DEFAULT_MASK) % 8, $this->level);
            }
        } else {
    ...
    protected function calcN1N3($length)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            if ($this->runLength[$i] >= 5) {
                $demerit += \OCA\JournalNotes\Vendor\N1 + ($this->runLength[$i] - 5);
            }
            if ($i & 1) {
                if ($i >= 3 and $i < $length - 2 and $this->runLength[$i] % 3 == 0) {
                    ...
                    if ($this->runLength[$i - 2] == $fact and $this->runLength[$i - 1] == $fact and $this->runLength[$i + 1] == $fact and $this->runLength[$i + 2] == $fact) {
                        if ($this->runLength[$i - 3] < 0 or $this->runLength[$i - 3] >= 4 * $fact) {
                            $demerit += \OCA\JournalNotes\Vendor\N3;
        ...
        for ($y = 0; $y < $width; ++$y) {
            ...
            for ($x = 0; $x < $width; ++$x) {
                if ($x > 0 and $y > 0) {
                    ...
                    if (($b22 | $w22 ^ 1) & 1) {
                        $demerit += \OCA\JournalNotes\Vendor\N2;
                    }
                }
    ...
    protected function mask($width, $frame, $level)
    {
        ...
        if (\OCA\JournalNotes\Vendor\QR_FIND_FROM_RANDOM !== \false) {
            $howManuOut = 8 - \OCA\JournalNotes\Vendor\QR_FIND_FROM_RANDOM % 9;
        ...
        foreach ($checked_masks as $i) {
            ...
            $demerit = (int) ((int) (\abs($blacks - 50) / 5) * \OCA\JournalNotes\Vendor\N4);
    ...
    protected function identifyMode($pos)
    {
        if ($pos >= \strlen($this->dataStr)) {
            return \OCA\JournalNotes\Vendor\QR_MODE_NL;
        }
        $c = $this->dataStr[$pos];
        if ($this->isdigitat($this->dataStr, $pos)) {
            return \OCA\JournalNotes\Vendor\QR_MODE_NM;
        } elseif ($this->isalnumat($this->dataStr, $pos)) {
            return \OCA\JournalNotes\Vendor\QR_MODE_AN;
        } elseif ($this->hint == \OCA\JournalNotes\Vendor\QR_MODE_KJ) {
            if ($pos + 1 < \strlen($this->dataStr)) {
                $d = $this->dataStr[$pos + 1];
                $word = \ord($c) << 8 | \ord($d);
                if ($word >= 0x8140 && $word <= 0x9ffc or $word >= 0xe040 && $word <= 0xebbf) {
                    return \OCA\JournalNotes\Vendor\QR_MODE_KJ;
                }
            }
        }
        return \OCA\JournalNotes\Vendor\QR_MODE_8B;
    }
    /**
     * eatNum
     * @return int run
    ...
    {
        $ln = $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_NM, $this->version);
        $p = 0;
        ...
        if ($mode == \OCA\JournalNotes\Vendor\QR_MODE_8B) {
        ...
        if ($mode == \OCA\JournalNotes\Vendor\QR_MODE_AN) {
        ...
        $this->items = $this->appendNewInputItem($this->items, \OCA\JournalNotes\Vendor\QR_MODE_NM, $run, \str_split($this->dataStr));
        return $run;
    }
    /**
     * eatAn
    ...
    {
        $la = $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_AN, $this->version);
        $ln = $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_NM, $this->version);
        $p = 1;
        ...
        $this->items = $this->appendNewInputItem($this->items, \OCA\JournalNotes\Vendor\QR_MODE_AN, $run, \str_split($this->dataStr));
        return $run;
    }
    /**
     * eatKanji
    ...
    {
        $p = 0;
        while ($this->identifyMode($p) == \OCA\JournalNotes\Vendor\QR_MODE_KJ) {
            $p += 2;
        }
        $this->items = $this->appendNewInputItem($this->items, \OCA\JournalNotes\Vendor\QR_MODE_KJ, $p, \str_split($this->dataStr));
        $run = $p;
        return $run;
    }
    /**
    ...
    protected function eat8()
    {
        ...
        while ($p < $dataStrLen) {
            $mode = $this->identifyMode($p);
            if ($mode == \OCA\JournalNotes\Vendor\QR_MODE_KJ) {
                break;
            }
            if ($mode == \OCA\JournalNotes\Vendor\QR_MODE_NM) {
                $q = $p;
                while ($this->isdigitat($this->dataStr, $q)) {
                    $q++;
                }
            ...
            } elseif ($mode == \OCA\JournalNotes\Vendor\QR_MODE_AN) {
                $q = $p;
        ...
        $run = $p;
        $this->items = $this->appendNewInputItem($this->items, \OCA\JournalNotes\Vendor\QR_MODE_8B, $run, \str_split($this->dataStr));
        return $run;
    }
    /**
     * splitString
    ...
    protected function splitString()
    {
        while (\strlen($this->dataStr) > 0) {
            ...
            switch ($mode) {
                case \OCA\JournalNotes\Vendor\QR_MODE_NM:
                    {
                        $length = $this->eatNum();
                        break;
                    }
                case \OCA\JournalNotes\Vendor\QR_MODE_AN:
                    {
                        $length = $this->eatAn();
                        break;
                    }
                case \OCA\JournalNotes\Vendor\QR_MODE_KJ:
                    {
                        if ($this->hint == \OCA\JournalNotes\Vendor\QR_MODE_KJ) {
    ...
    protected function encodeModeNum($inputitem, $version)
    {
        ...
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_NM, $version), $inputitem['size']);
        for ($i = 0; $i < $words; ++$i) {
    ...
    protected function encodeModeAn($inputitem, $version)
    {
        ...
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_AN, $version), $inputitem['size']);
        for ($i = 0; $i < $words; ++$i) {
    ...
    protected function encodeMode8($inputitem, $version)
    {
        ...
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_8B, $version), $inputitem['size']);
    ...
    protected function encodeModeKanji($inputitem, $version)
    {
        ...
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(\OCA\JournalNotes\Vendor\QR_MODE_KJ, $version), (int) ($inputitem['size'] / 2));
    ...
    protected function encodeBitStream($inputitem, $version)
    {
        ...
        if ($inputitem['size'] > $words) {
            ...
            switch ($inputitem['mode']) {
                ...
                case \OCA\JournalNotes\Vendor\QR_MODE_8B:
                    {
                ...
                case \OCA\JournalNotes\Vendor\QR_MODE_ST:
                    {
    ...
    {
        if ($size > \OCA\JournalNotes\Vendor\MAX_STRUCTURED_SYMBOLS) {
            return -1;
        }
        if ($index <= 0 or $index > \OCA\JournalNotes\Vendor\MAX_STRUCTURED_SYMBOLS) {
            return -1;
        }
        $buf = array($size, $index, $parity);
        $entry = $this->newInputItem(\OCA\JournalNotes\Vendor\QR_MODE_ST, 3, $buf);
    ...
    protected function calcParity($items)
    {
        ...
        foreach ($items as $item) {
            if ($item['mode'] != \OCA\JournalNotes\Vendor\QR_MODE_ST) {
    ...
    protected function estimateBitStreamSize($items, $version)
    {
        ...
        foreach ($items as $item) {
            switch ($item['mode']) {
                ...
                case \OCA\JournalNotes\Vendor\QR_MODE_ST:
                    {
                        return \OCA\JournalNotes\Vendor\STRUCTURE_HEADER_BITS;
                    }
                default:
                    {
                        return 0;
    ...
    {
        return $this->capacity[$version][\OCA\JournalNotes\Vendor\QRCAP_WORDS] - $this->capacity[$version][\OCA\JournalNotes\Vendor\QRCAP_EC][$level];
    }
    /**
    ...
    {
        return $this->capacity[$version][\OCA\JournalNotes\Vendor\QRCAP_EC][$level];
    }
    /**
    ...
    {
        return $this->capacity[$version][\OCA\JournalNotes\Vendor\QRCAP_WIDTH];
    }
    /**
    ...
    {
        return $this->capacity[$version][\OCA\JournalNotes\Vendor\QRCAP_REMINDER];
    }
    /**
    ...
    {
        for ($i = 1; $i <= \OCA\JournalNotes\Vendor\QRSPEC_VERSION_MAX; ++$i) {
            $words = $this->capacity[$i][\OCA\JournalNotes\Vendor\QRCAP_WORDS] - $this->capacity[$i][\OCA\JournalNotes\Vendor\QRCAP_EC][$level];
            if ($words >= $size) {
                return $i;
            }
        }
    ...
    {
        if ($mode == \OCA\JournalNotes\Vendor\QR_MODE_ST) {
            return 0;
        }
        if ($version <= 9) {
            $l = 0;
    ...
    {
        if ($version < 7 or $version > \OCA\JournalNotes\Vendor\QRSPEC_VERSION_MAX) {
            return 0;
        }
    ...
    {
        $width = $this->capacity[$version][\OCA\JournalNotes\Vendor\QRCAP_WIDTH];
    ...
    {
        if ($version < 1 or $version > \OCA\JournalNotes\Vendor\QRSPEC_VERSION_MAX) {
            return NULL;
        }
