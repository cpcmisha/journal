...
class TCPDI extends FPDF_TPL
{
    ...
    public function importAnnotations($pageno)
    {
        ...
        if (\is_array($annots) && $annots[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT && \is_array($annots[1]) && $annots[1][0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY && \is_array($annots[1][1]) && \count($annots[1][1] > 1)) {
    ...
    public function importAnnotation($annotation)
    {
        $fn = $this->current_filename;
        $old_id = $annotation[1];
        $value = array(\OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF, $old_id, 0);
        if (!isset($this->_don_obj_stack[$fn][$old_id])) {
            $this->_newobj(\false, \true);
    ...
    {
        if ($adjustPageSize == \true && \is_null($_x) && \is_null($_y)) {
            ...
            foreach ($this->parsers as $filename => $p) {
                ...
                if (isset($this->_obj_stack[$filename]) && \is_array($this->_obj_stack[$filename])) {
                    while (($n = \key($this->_obj_stack[$filename])) !== null) {
                        ...
                        if ($nObj[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM) {
    ...
    public function _putformxobjects()
    {
        ...
        foreach ($this->tpls as $tplidx => $tpl) {
            ...
            if (isset($tpl['resources'])) {
                ...
                if (isset($this->_res['tpl'][$tplidx]['images']) && \count($this->_res['tpl'][$tplidx]['images']) || isset($this->_res['tpl'][$tplidx]['tpls']) && \count($this->_res['tpl'][$tplidx]['tpls'])) {
                    $this->_out('/XObject <<');
    ...
    {
        switch ($value[0]) {
            case \OCA\JournalNotes\Vendor\PDF_TYPE_STRING:
                if ($this->encrypted) {
                ...
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM:
                if ($this->encrypted) {
                    $value[2][1] = $this->_encrypt_data($this->_current_obj_id, $value[2][1]);
                    $value[1][1]['/Length'] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC, \strlen($value[2][1]));
                }
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_HEX:
                if ($this->encrypted) {
        ...
        switch ($value[0]) {
            case \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN:
                $this->_straightOut('/' . $value[1] . ' ');
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC:
            case \OCA\JournalNotes\Vendor\PDF_TYPE_REAL:
                ...
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY:
                ...
                $this->_out(']');
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY:
                // A dictionary.
                $this->_straightOut('<<');
                ...
                $this->_straightOut('>>');
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF:
            ...
            case \OCA\JournalNotes\Vendor\PDF_TYPE_HEX:
                $this->_straightOut('<' . $value[1] . '>');
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_BOOLEAN:
                $this->_straightOut($value[1] ? 'true ' : 'false ');
                break;
            case \OCA\JournalNotes\Vendor\PDF_TYPE_NULL:
                // The null object.
    ...
    protected function _straightOut($s)
    {
        if ($this->state == 2) {
            if ($this->inxobj) {
                ...
                // set page data
                $this->setPageBuffer($this->page, $s, \true);
            }
        } elseif ($this->state > 0) {
            // set general data
            $this->setBuffer($s);
