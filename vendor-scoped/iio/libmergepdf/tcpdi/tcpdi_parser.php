 ...
 * @version 1.1
 */
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_NULL')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_NULL', 0);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC', 1);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN', 2);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_HEX')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_HEX', 3);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_STRING')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_STRING', 4);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY', 5);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY', 6);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_OBJDEC')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_OBJDEC', 7);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF', 8);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT', 9);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_STREAM')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_STREAM', 10);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_BOOLEAN')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_BOOLEAN', 11);
}
if (!\defined('OCA\JournalNotes\Vendor\PDF_TYPE_REAL')) {
    \define('OCA\JournalNotes\Vendor\PDF_TYPE_REAL', 12);
}
/**
 * @class tcpdi_parser
 * This is a PHP class for parsing PDF documents.<br>
...
class tcpdi_parser
{
    ...
    function readPages()
    {
        ...
        if ($objref == null || $objref[0] !== \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
            // Offset not found.
            return;
        }
        $dict = $this->getObjectVal($objref);
        if ($dict[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT && $dict[1][0] == \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY) {
            // Dict wrapped in an object
            $dict = $dict[1];
        }
        if ($dict[0] !== \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY) {
            return;
        }
        $this->pages = array();
        if (isset($dict[1]['/Kids'])) {
            $v = $dict[1]['/Kids'];
            if ($v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
                foreach ($v[1] as $ref) {
    ...
    protected function decodeXref($startxref, $xref = array())
    {
        ...
        if (\preg_match('/trailer[\s]*<<(.*)>>[\s\r\n]+(?:[%].*[\r\n]+)*startxref[\s\r\n]+/isU', $this->pdfdata, $matches, \PREG_OFFSET_CAPTURE, $xoffset) > 0) {
            ...
            if (!isset($xref['trailer']) or empty($xref['trailer'])) {
                ...
                $xref['trailer'] = array();
                $xref['trailer'][0] = \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY;
                $xref['trailer'][1] = array();
                // parse trailer_data
                if (\preg_match('/Size[\s]+([0-9]+)/i', $trailer_data, $matches) > 0) {
                    $xref['trailer'][1]['/Size'] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC, \intval($matches[1]));
                }
                if (\preg_match('/Root[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) {
                    $xref['trailer'][1]['/Root'] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF, \intval($matches[1]), \intval($matches[2]));
                }
                if (\preg_match('/Encrypt[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) {
                    $xref['trailer'][1]['/Encrypt'] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF, \intval($matches[1]), \intval($matches[2]));
                }
                if (\preg_match('/Info[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) {
                    $xref['trailer'][1]['/Info'] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF, \intval($matches[1]), \intval($matches[2]));
                }
                if (\preg_match('/ID[\s]*[\[][\s]*[<]([^>]*)[>][\s]*[<]([^>]*)[>]/i', $trailer_data, $matches) > 0) {
                    $xref['trailer'][1]['/ID'] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY, array());
                    $xref['trailer'][1]['/ID'][1][0] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_HEX, $matches[1]);
                    $xref['trailer'][1]['/ID'][1][1] = array(\OCA\JournalNotes\Vendor\PDF_TYPE_HEX, $matches[2]);
                }
            }
    ...
    protected function decodeXrefStream($startxref, $xref = array())
    {
        ...
        $columns = 1;
        // Default as per PDF 32000-1:2008.
        $predictor = 1;
        // Default as per PDF 32000-1:2008.
        foreach ($keys as $k => $key) {
            $v = $sarr[$key];
            if ($key == '/Type' and ($v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN and $v[1] == 'XRef')) {
                $valid_crs = \true;
            } elseif ($key == '/Index' and ($v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY and \count($v[1]) >= 2)) {
                // first object number in the subsection
                $index_first = \intval($v[1][0][1]);
                // number of entries in the subsection
                $index_entries = \intval($v[1][1][1]);
            } elseif ($key == '/Prev' and $v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC) {
                // get previous xref offset
                $prevxref = \intval($v[1]);
            } elseif ($key == '/W' and $v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
                // number of bytes (in the decoded stream) of the corresponding field
                $wb = array();
                $wb[0] = \intval($v[1][0][1]);
                $wb[1] = \intval($v[1][1][1]);
                $wb[2] = \intval($v[1][2][1]);
            } elseif ($key == '/DecodeParms' and $v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY) {
                $decpar = $v[1];
                foreach ($decpar as $kdc => $vdc) {
                    if ($kdc == '/Columns' and $vdc[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC) {
                        $columns = \intval($vdc[1]);
                    } elseif ($kdc == '/Predictor' and $vdc[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC) {
        ...
        if ($valid_crs and isset($xrefcrs[1][3][0])) {
            ...
            foreach ($sdata as $k => $row) {
                ...
                for ($i = 1; $i <= $columns; ++$i) {
                    ...
                    switch ($predictor) {
                        case 1:
                        // No prediction (equivalent to PNG None)
                        case 10:
                            {
    ...
    protected function getRawStream($offset, $length)
    {
        ...
        $obj = array();
        $obj[] = \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM;
    ...
    protected function getRawObject($offset = 0, $data = null)
    {
            ...
            case '/':
                {
                    // \x2F SOLIDUS
                    // name object
                    $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN;
                    ++$offset;
                    $length = \strcspn($data, "\x00\t\n\f\r ()<>[]{}/%", $offset);
            ...
            case ')':
                {
                    ...
                    $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_STRING;
                    ++$offset;
                    $strpos = $offset;
                    if ($char == '(') {
                        $open_bracket = 1;
            ...
            case ']':
                {
                    ...
                    // array object
                    $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY;
                    ++$offset;
                    if ($char == '[') {
                        // get array content
                        $objval = array();
            ...
            case '>':
                {
                    ...
                    if (isset($data[$offset + 1]) and $data[$offset + 1] == $char) {
                        // dictionary object
                        $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY;
                        if ($char == '<') {
                        ...
                        $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_HEX;
                        ++$offset;
            ...
            default:
                {
                    ...
                    switch ($frag) {
                        ...
                        case 'null':
                            // null object
                            $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_NULL;
                            $offset += 4;
                            $objval = 'null';
                            break;
                        case 'true':
                            // boolean true object
                            $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_BOOLEAN;
                            $offset += 4;
                            $objval = \true;
                            break;
                        case 'fals':
                        ...
                        default:
                            if (\preg_match('/^([0-9]+)[\s]+([0-9]+)[\s]+([Robj]{1,3})/i', \substr($data, $offset, 33), $matches) == 1) {
                                if ($matches[3] == 'R') {
                                    ...
                                    $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF;
                                    $offset += \strlen($matches[0]);
                                    $objval = array(\intval($matches[1]), \intval($matches[2]));
                                } elseif ($matches[3] == 'obj') {
                                    // object start
                                    $objtype = \OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT;
                                ...
                                $objtype = \intval($objval) != $objval ? \OCA\JournalNotes\Vendor\PDF_TYPE_REAL : \OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC;
                                $offset += $numlen;
                            }
                            unset($matches);
                            break;
        ...
        $obj = array();
        $obj[] = $objtype;
        if ($objtype == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF && \is_array($objval)) {
    ...
    protected function getIndirectObject($obj_ref, $offset = 0, $decoding = \true)
    {
        ...
        // object main index
        do {
            if ($i > 0 and isset($objdata[$i - 1][0]) and $objdata[$i - 1][0] == \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY and \array_key_exists('/Length', $objdata[$i - 1][1])) {
                // Stream - get using /Length in stream's dict
                $lengthobj = $objdata[$i - 1][1]['/Length'];
                if ($lengthobj[0] === \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
                    $lengthobj = $this->getObjectVal($lengthobj);
                    if ($lengthobj[0] === \OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT) {
            ...
            if ($decoding and $element[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM and isset($objdata[$i - 1][0]) and $objdata[$i - 1][0] == \OCA\JournalNotes\Vendor\PDF_TYPE_DICTIONARY) {
    ...
    {
        if ($obj[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
            if (\strpos($obj[1], '_') !== \false) {
                $key = \explode('_', $obj[1]);
            } else {
                $key = array($obj[1], $obj[2]);
            }
            $ret = array(0 => \OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT, 'obj' => $key[0], 'gen' => $key[1]);
            ...
            if (isset($this->objects[$key[0]][$key[1]])) {
                ...
                if (!isset($this->objstreams[$objs[0]][$objs[1]])) {
                    ...
                    $objstream = $this->getObjectVal(array(\OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF, $objs[0], $objs[1]));
                    $decoded = $this->decodeStream($objstream[1][1], $objstream[2][1]);
                    $this->objstreams[$objs[0]][$objs[1]] = $decoded[0];
                    // Store just the data, in case we need more from this objstream
                    // Free memory
                    unset($objstream);
                    unset($decoded);
                }
            ...
            if (!\is_null($object)) {
                $ret[1] = $object[0];
                if (isset($object[1][0]) && $object[1][0] == \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM) {
                    $ret[0] = \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM;
                    $ret[2] = $object[1];
                }
                return $ret;
            }
    ...
    {
        $objref = array(\OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF, $key[0], $key[1]);
        $obj = $this->getObjectVal($objref);
        if ($obj[0] !== \OCA\JournalNotes\Vendor\PDF_TYPE_STREAM || !isset($obj[1][1]['/First'][1])) {
            // Not a valid object stream dictionary - skip it.
            return;
        }
        $stream = $this->decodeStream($obj[1][1], $obj[2][1]);
        // Decode object stream, as we need the first bit
        $first = \intval($obj[1][1]['/First'][1]);
        $ints = \preg_split('/\s/', \substr($stream[0], 0, $first));
        // Get list of object / offset pairs
    ...
    private function findObjectOffsets()
    {
        ...
        if (\preg_match_all('/(*ANYCRLF)^[\s]*([0-9]+)[\s]+([0-9]+)[\s]+obj/im', $this->pdfdata, $matches, \PREG_OFFSET_CAPTURE) >= 1) {
            ...
            foreach ($matches[0] as $match) {
                $offset = $match[1] + \strspn($match[0], "\x00\t\n\f\r ");
                if ($offset < $laststreamend) {
                    // Contained within another stream, skip it.
                    continue;
                }
                $this->objoffsets[\trim($match[0])] = $offset;
    ...
    protected function decodeStream($sdic, $stream)
    {
        ...
        $filters = array();
        foreach ($sdic as $k => $v) {
            if ($v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN) {
                if ($k == '/Length' and $v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_NUMERIC) {
                    ...
                    if ($v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN) {
                        // single filter
                        $filters[] = $v[1];
                    } elseif ($v[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
                        // array of filters
                        foreach ($v[1] as $flt) {
                            if ($flt[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN) {
                                $filters[] = $flt[1];
                            }
                        }
                    }
    ...
    private function _getPageResources($obj)
    {
        ...
        if (isset($obj[1][1]['/Resources'])) {
            ...
            if ($res[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJECT) {
                return $res[1];
            }
            return $res;
    ...
    private function _getPageAnnotations($obj)
    {
        ...
        if ($annots[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
    ...
    {
        $contents = array();
        if ($content_ref[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
            $content = $this->getObjectVal($content_ref);
            if ($content[1][0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
                $contents = $this->_getPageContent($content[1]);
            } else {
                $contents[] = $content;
            }
        } elseif ($content_ref[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
    ...
    private function _rebuildContentStream($obj)
    {
        ...
        if (isset($obj[1][1]['/Filter'])) {
            ...
            if ($_filter[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
                $tmpFilter = $this->getObjectVal($_filter);
                $_filter = $tmpFilter[1];
            }
            if ($_filter[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_TOKEN) {
                $filters[] = $_filter;
            } elseif ($_filter[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
                $filters = $_filter[1];
            }
        }
        $stream = $obj[2][1];
    ...
    public function getPageBox($page, $box_index, $k)
    {
        ...
        if (!\is_null($box) && $box[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_OBJREF) {
            $tmp_box = $this->getObjectVal($box);
            $box = $tmp_box[1];
        }
        if (!\is_null($box) && $box[0] == \OCA\JournalNotes\Vendor\PDF_TYPE_ARRAY) {
            $b =& $box[1];
