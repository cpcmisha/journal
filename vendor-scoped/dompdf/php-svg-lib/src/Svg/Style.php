...
class Style
{
    ...
    public function fromStyleSheets(AbstractTag $tag, $attributes)
    {
        ...
        $outputFormat = \OCA\JournalNotes\Vendor\Sabberworm\CSS\OutputFormat::createCompact();
        $styles = array();
                        ...
                        foreach ($class as $_class) {
                            if ($_selector === ".{$_class}") {
                                ...
                                foreach ($_decl->getRules() as $_rule) {
                                    ...
                                    if ($value instanceof \OCA\JournalNotes\Vendor\Sabberworm\CSS\Value\Value) {
    ...
    static function getQuad($color, $percent = \false)
    {
        ...
        foreach (array_keys($quad) as $c) {
            ...
            } else if ($quad[$c][strlen($quad[$c]) - 1] === "%") {
