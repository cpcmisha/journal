 ...
 */
namespace OCA\JournalNotes\Vendor\Dompdf;

use DOMDocument;
use DOMNode;
use OCA\JournalNotes\Vendor\Dompdf\Adapter\CPDF;
use DOMXPath;
use OCA\JournalNotes\Vendor\Dompdf\Frame\Factory;
use OCA\JournalNotes\Vendor\Dompdf\Frame\FrameTree;
use OCA\JournalNotes\Vendor\Dompdf\Image\Cache;
use OCA\JournalNotes\Vendor\Dompdf\Css\Stylesheet;
use OCA\JournalNotes\Vendor\Dompdf\Helpers;
use OCA\JournalNotes\Vendor\Masterminds\HTML5;
/**
...
class Dompdf
{
    ...
    public function render()
    {
        ...
        if (\OCA\JournalNotes\Vendor\Dompdf\Helpers::lengthEqual($canvasWidth, $size[2]) === \false || \OCA\JournalNotes\Vendor\Dompdf\Helpers::lengthEqual($canvasHeight, $size[3]) === \false) {
            $this->canvas = CanvasFactory::get_instance($this, $this->paperSize, $this->paperOrientation);
    ...
    public function setPaper($size, string $orientation = "portrait"): self
    {
        ...
        if (\OCA\JournalNotes\Vendor\Dompdf\Helpers::lengthEqual($current_size[2], $new_size[2]) === \false || \OCA\JournalNotes\Vendor\Dompdf\Helpers::lengthEqual($current_size[3], $new_size[3]) === \false) {
            $this->canvas = CanvasFactory::get_instance($this, $this->paperSize, $this->paperOrientation);
        }
        return $this;
    }
