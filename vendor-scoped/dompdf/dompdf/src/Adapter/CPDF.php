...
namespace OCA\JournalNotes\Vendor\Dompdf\Adapter;

use OCA\JournalNotes\Vendor\Dompdf\Canvas;
use OCA\JournalNotes\Vendor\Dompdf\Dompdf;
use OCA\JournalNotes\Vendor\Dompdf\Exception;
use OCA\JournalNotes\Vendor\Dompdf\FontMetrics;
use OCA\JournalNotes\Vendor\Dompdf\Helpers;
use OCA\JournalNotes\Vendor\Dompdf\Image\Cache;
use OCA\JournalNotes\Vendor\FontLib\Exception\FontNotFoundException;
/**
...
class CPDF implements Canvas
{
    ...
    public function __construct($paper = "letter", string $orientation = "portrait", ?Dompdf $dompdf = null)
    {
        ...
        $this->_pdf = new \OCA\JournalNotes\Vendor\Dompdf\Cpdf($size, \true, $this->_dompdf->getOptions()->getFontCache(), $this->_dompdf->getOptions()->getTempDir());
    ...
    public function select($x, $y, $w, $h, $font, $size, $color = [0, 0, 0], $opts = [])
    {
        ...
        $ft = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_CHOICE;
        $ff = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_CHOICE_COMBO;
    ...
    public function textarea($x, $y, $w, $h, $font, $size, $color = [0, 0, 0])
    {
        ...
        $ft = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT;
        $ff = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT_MULTILINE;
    ...
    public function input($x, $y, $w, $h, $type, $font, $size, $color = [0, 0, 0])
    {
        ...
        switch ($type) {
            ...
            case 'password':
                $ft = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT;
                $ff = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT_PASSWORD;
                break;
            case 'submit':
                $ft = \OCA\JournalNotes\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_BUTTON;
                break;
        }
        $pdf->addFormField($ft, rand(), $x, $this->y($y) - $h, $x + $w, $this->y($y), $ff, $size, $color);
    }
