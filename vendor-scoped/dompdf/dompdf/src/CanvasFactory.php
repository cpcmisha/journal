...
class CanvasFactory
{
    ...
    static function get_instance(Dompdf $dompdf, $paper, string $orientation, ?string $class = null)
    {
        ...
        if (isset($class) && class_exists($class, \false)) {
            ...
            $class = "OCA\\JournalNotes\\Vendor\\Dompdf\\Adapter\\PDFLib";
        } else if (class_exists($backend, \false)) {
            $class = $backend;
        } elseif ($backend === "gd" && extension_loaded('gd')) {
            $class = "OCA\\JournalNotes\\Vendor\\Dompdf\\Adapter\\GD";
        } else {
            $class = "OCA\\JournalNotes\\Vendor\\Dompdf\\Adapter\\CPDF";
        }
        $instance = new $class($paper, $orientation, $dompdf);
        $class_interfaces = class_implements($class, \false);
        if (!$class_interfaces || !in_array("OCA\\JournalNotes\\Vendor\\Dompdf\\Canvas", $class_interfaces)) {
            $class = "OCA\\JournalNotes\\Vendor\\Dompdf\\Adapter\\CPDF";
            $instance = new $class($paper, $orientation, $dompdf);
        }
        return $instance;
